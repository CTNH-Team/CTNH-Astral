...
public class AstralGrass extends GrassBlock {

    public AstralGrass(Properties properties) {
        super(properties);
    }
    ...
    public void randomTick(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
        if (!canBeGrass(state, level, pos)) {
            ...
            level.setBlockAndUpdate(pos, AstralBlocks.ASTRAL_DIRT.get().defaultBlockState());
        } else {
            ...
            if (level.getMaxLocalRawBrightness(pos.above()) >= 9) {
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    if (level.getBlockState(blockpos).is(AstralBlocks.ASTRAL_DIRT.get()) &&
                            canPropagate(blockstate, level, blockpos)) {
                        level.setBlockAndUpdate(blockpos, (BlockState) blockstate.setValue(SNOWY,
                                level.getBlockState(blockpos.above()).is(Blocks.SNOW)));
                    }
                }
            }
        }
